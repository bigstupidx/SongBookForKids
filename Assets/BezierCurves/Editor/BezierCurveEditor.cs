	...
	public override void OnInspectorGUI()
	{
		...
		if(showPoints)
		{
			...
			if(GUILayout.Button("Add Point"))
			{
				...
				Vector3 localPos = curve[pointsProp.arraySize-1].transform.localPosition;
				pointObject.transform.localPosition = localPos + (Vector3.right + Vector3.up) * 2f;
				BezierPoint newPoint = pointObject.AddComponent<BezierPoint>();

				newPoint.curve = curve;
				newPoint.handle1 = Vector3.right;
				newPoint.handle2 = -Vector3.right;
